...
const SOLUTIONS={
  17:{
    ...
    azimuthDeg:277.3334,azimuth:"277,3°",actualDirection:"O 7,3° N",
  ...
  771:{
    ...
    azimuthDeg:171.2378,azimuth:"171,2°",actualDirection:"S 8,8° E",
...
function inheritedAltitudeHTML(s){
  ...
  <div class="zenith-substep"><b>A · Corrección de índice</b><p>El enunciado dice <b>«CI = 1′ a la derecha»</b>. Esto significa que, al hacer coincidir en el sextante el horizonte visto directamente y su imagen reflejada, la marca queda 1′ a la derecha del 0° de la escala.</p><div class="zenith-index-rule"><b>NO SON PUNTOS CARDINALES</b><span>«Derecha» e «izquierda» solo indican a qué lado del <b>0° del sextante</b> queda la marca. No significan Este, Oeste, Norte, Sur, babor ni estribor.</span></div><div class="zenith-index-signs"><div><b>DERECHA</b><span>CI positiva</span><strong>+1,0′ · SE SUMA</strong></div><div><b>IZQUIERDA</b><span>CI negativa</span><strong>−1,0′ · SE RESTA</strong></div></div><p>En esta pregunta queda a la <b>derecha</b>, por lo que la lectura del sextante es 1′ menor de lo que debería ser. Para corregirla usamos <b>CI = +1,0′</b> y la sumamos a la altura instrumental.</p>${resultFrame("ao","29°31,7′ + 0°01,0′ = 29°32,7′")}</div>
...
function zenithDistanceProblemTypeHTML(q){
  ...
  const orientationReminder=q.id===17?`<div class="zenith-memory"><b>RECUERDA:</b> <span>|z| = 90° − av</span>. Esta resta solo da el <b>tamaño</b> del arco. Para saber si el Sol queda hacia el Norte, Sur, Este u Oeste hay que conocer su <b>azimut</b>.</div><p class="zenith-small">No deduzcas la orientación mirando únicamente la latitud, la declinación o el signo obtenido en la resta.</p>`:`<div class="zenith-memory"><b>RECUERDA:</b> <span>z = 90° − av</span>. Después se añade el signo: <b>cara al Norte → z positiva</b> · <b>cara al Sur → z negativa</b>.</div><p class="zenith-small">El signo de z expresa hacia qué lado del cenit queda el astro. No significa que la altura del Sol sea negativa.</p>`;
  return `<div class="zenith-type"><b>TIPO DE PROBLEMA · DISTANCIA CENITAL (z) Y ORIENTACIÓN</b><p>Lo reconoces porque pide la <b>distancia cenital z</b>: el arco que va desde el <b>cenit Z</b> del observador hasta el Sol. La altura verdadera <b>av</b> se mide desde el horizonte hasta el Sol; por eso ambas completan siempre 90°.</p>${orientationReminder}</div>`;
}
...
function zenithDistanceSolutionHTML(q){
  ...
  const sourceText=s.inherited?`La latitud es Norte y la declinación del Sol es Sur. El Sol se observa con azimut aproximado <b>${s.azimuth}</b>, muy cerca del Sur; por eso aquí sí corresponde decir <b>cara al Sur</b> y escribir z con signo negativo.`:`<b>La resta anterior no permite decidir Norte, Sur, Este u Oeste.</b> Para conocer la orientación necesitamos el azimut. En el ejercicio siguiente del examen original se da, para este mismo instante, <b>P = 69°29,8′ W</b>. Al comprobar el triángulo de posición se obtiene <b>azimut ≈ ${s.azimuth}</b>: el Sol está al Oeste y ligeramente hacia el Norte (<b>${s.actualDirection}</b>), no cara al Sur.`;
  const orientationResult=q.id===17?`${resultFrame("ORIENTACIÓN",`azimut ≈ ${s.azimuth} · ${s.actualDirection}`,"#c77816","#fff7df")}<div class="zenith-official-warning"><b>ATENCIÓN · INCOHERENCIA EN LA RESPUESTA OFICIAL</b><p>El valor numérico de la opción C sí coincide: <b>|z| = ${s.z}</b>. Sin embargo, su texto «cara al Sur» no concuerda con la posición real del Sol. Ninguna opción combina el valor correcto con la orientación correcta. Conservamos <b>C</b> únicamente porque es la respuesta marcada en el examen oficial.</p></div>`:`${resultFrame("z",`${s.signedZ} · cara al ${s.face}`,"#7657a5","#f4effb")}<div class="zenith-result"><b>RESULTADO FINAL · ${s.answer} · z = ${s.signedZ} cara al ${s.face}</b></div>`;
  return `<details class="card zenith-solution" open><summary><b>🟢 Resolución completa</b></summary><div class="zenith-body">
    ...
    <section class="zenith-step"><span class="zenith-step-number">PASO 3</span><h3>COMPRUEBA LA ORIENTACIÓN</h3><p>${sourceText}</p>${orientationResult}<div class="zenith-check"><b>Comprobación del valor:</b> ${s.check}. La altura verdadera y el valor absoluto de la distancia cenital suman exactamente 90°.</div></section>
...
function zenithDistanceQuickHTML(q){
  const s=SOLUTIONS[q.id];if(!s)return "";
  const inherited=s.inherited?`<div class="zenith-quick-context"><b>Datos heredados que necesitas:</b> 19/11/2016 · ${s.time} · l = ${s.lat} · L = ${s.lon} · d = ${s.dec} · ai = ${s.ai} · e = ${s.eye} · CI = 1′ a la derecha → <b>+1,0′</b> · limbo inferior del Sol.</div><details class="zenith-quick-almanac"><summary>Abrir la hoja real del Almanaque ↓</summary><div class="zenith-almanac-scroll"><img src="assets/almanaque/2016/2016_p387_correcciones_altura_verdadera.png" alt="Página 387 del Almanaque Náutico de 2016 con las tablas de corrección de alturas" loading="lazy"></div></details>`:"";
  const orientation=q.id===17?`<li>Con <b>P = 69°29,8′ W</b>, el azimut es aproximadamente <b>${s.azimuth}</b>: dirección <b>${s.actualDirection}</b>.</li><li><b>El examen marca C</b> por el valor ${s.z}, pero «cara al Sur» es incoherente.</li>`:`<li>El azimut aproximado es <b>${s.azimuth}</b>: el Sol está cara al Sur.</li><li><b>${s.answer} · z = ${s.signedZ} cara al Sur</b>.</li>`;
  return `<details class="quick-reminder"><summary>⚡ Recordatorio rápido</summary><div class="quick-reminder-body"><b>Reconoce:</b> piden distancia cenital y su orientación.${inherited}<ol>${s.inherited?`<li>Corrige ai con la página 387: <b>av = ${s.av}</b>.</li>`:""}<li><b>|z| = 90° − av = 90° − ${s.av} = ${s.z}</b>.</li>${orientation}</ol></div></details>`;
}

function zenithGraphicHTML(s,id){
  return `<section class="zenith-graphic"><div class="zenith-graphic-head"><div><h3>ESFERA CELESTE INTERACTIVA · interpreta lo calculado</h3><p>Arrástrala con el ratón. Las líneas de la parte posterior se ven más tenues para que la Tierra no parezca colocada encima de ellas.</p></div><button type="button" class="action" data-zenith-reset>Recuperar vista</button></div><canvas data-zenith-sphere data-question-id="${id}" role="img" aria-label="Esfera celeste tridimensional con el observador, el horizonte, el cenit, el Sol, la altura verdadera ${s.av}, la distancia cenital ${s.z} y el azimut ${s.azimuth}"></canvas><div class="zenith-legend"><div style="--c:#c93645"><b>Horizonte del observador</b><span>El círculo rojo pasa por N, E, S y W.</span></div><div style="--c:#7657a5"><b>av = ${s.av}</b><span>Arco morado: del horizonte al Sol.</span></div><div style="--c:#248a55"><b>|z| = ${s.z}</b><span>Arco verde: del Sol al cenit Z.</span></div><div style="--c:#c77816"><b>Azimut ≈ ${s.azimuth}</b><span>Dirección aproximada: ${s.actualDirection}.</span></div></div><div class="zenith-graphic-note"><b>Cómo leerlo:</b> av y |z| están sobre el mismo semicírculo vertical y completan 90°. Ese semicírculo está orientado hacia el <b>azimut real aproximado ${s.azimuth}</b>; por eso permite comprobar hacia qué parte del horizonte queda el Sol.</div></section>`;
}
...
function initZenithDistanceGraphics(root=document){
  root.querySelectorAll("canvas[data-zenith-sphere]").forEach(canvas=>{
    ...
    function draw(){
      ...
      const az=rad(s.azimuthDeg||180),hx=Math.sin(az),hy=Math.cos(az),horizon=circle(t=>[Math.sin(t),Math.cos(t),0]),vertical=circle(t=>[Math.sin(t)*hx,Math.sin(t)*hy,Math.cos(t)]);
      drawCurve(vertical,"#9fb1bd",1.7,.1,[6,5]);drawCurve(horizon,"#c93645",4,.2);
      const a=rad(s.avDeg),altArc=Array.from({length:90},(_,i)=>{const t=a*i/89;return[hx*Math.cos(t),hy*Math.cos(t),Math.sin(t)]}),zArc=Array.from({length:120},(_,i)=>{const t=a+(Math.PI/2-a)*i/119;return[hx*Math.cos(t),hy*Math.cos(t),Math.sin(t)]});
      ...
      const zen=[0,0,1],sun=[hx*Math.cos(a),hy*Math.cos(a),Math.sin(a)],observer=[0,0,.105],zp=project(zen),sp=project(sun),op=project(observer);

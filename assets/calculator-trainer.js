...
function mount(root){
 ...
 if(inlineTrainer){root.classList.add("calc-inline-trainer");const updateDock=function(){if(!root.isConnected){window.removeEventListener("scroll",updateDock);window.removeEventListener("resize",updateDock);return}const rect=root.getBoundingClientRect();root.classList.toggle("calc-docked",rect.top<=window.innerHeight*.72)};window.addEventListener("scroll",updateDock,{passive:true});window.addEventListener("resize",updateDock);updateDock()}

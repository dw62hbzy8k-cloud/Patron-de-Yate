...
function trainerMarkup(guideId){
 ...
 const subtitle=is296?"Cálculo interactivo de esta pregunta: altura estimada del Sol.":guideId?"Cálculo interactivo completo de la altura estimada del Sol.":"Convertir grados decimales en grados y minutos.";
 const objective=is296?"Objetivo de esta pregunta: ae = 51°14,1′":guideId?"Objetivo: ae = 44°53,7′":"Objetivo: 44,895188° = 44°53,7′";
 const steps=is296?"<li>Introducir l, d y P en la fórmula.</li><li>Calcular sen(ae) y recuperar ae con SHIFT → SIN.</li><li>Pasar ae a grados, minutos y segundos con °′″.</li>":guideId?"<li>Introducir l, d y P en la fórmula.</li><li>Recuperar ae con SHIFT → SIN.</li><li>Convertir y redondear el resultado final.</li>":"<li>Quitar los 44 grados enteros.</li><li>Multiplicar la parte decimal por 60 para convertirla en minutos.</li><li>Redondear 53,71128′ a 53,7′ y detenerse.</li>";
 const initial=is296?"Empieza por SIN: vas a introducir la fórmula real con los datos de esta pregunta.":guideId?"Empieza por SIN: vas a introducir la fórmula real con l, d y P.":"La pantalla ya contiene 44,895188. Empieza por la operación indicada.";
...
function mount(root){
 ...
 const height296=guideId==="296",heightGuide=guideId==="1"||height296;
 const expected=height296?height296Expected:heightGuide?heightExpected:conversionExpected,directions=height296?height296Directions:heightGuide?heightDirections:conversionDirections,storageKey=height296?HEIGHT_296_STORAGE_KEY:heightGuide?HEIGHT_STORAGE_KEY:STORAGE_KEY;
 const finalHeightText=height296?"ae = 51°14′06,4″, que en las respuestas es 51°14,1′.":"ae = 44°53,7′.";
 const milestones=height296?[{at:61,value:"0.779722"},{at:66,value:"51.235123"},{at:67,value:"51°14′06,4″"}]:[{at:61,value:"0.7058120802"},{at:66,value:"44.8951881663"},{at:70,value:"0.8951881663"},{at:74,value:"53.7112899782"}];
 ...
 function stepIndex(){if(heightGuide){if(cursor<61)return 0;if(cursor<66)return 1;return 2}if(cursor<=3)return 0;if(cursor<=7)return 1;return 2}
 ...
 function reset(){cursor=0;history=[];expression=startExpression;display=startValue;stored=Number(startValue);pending=null;typing=false;completed=false;feedbackEl.className="calc-feedback";feedbackEl.textContent=height296?"Empieza por SIN: vas a introducir la fórmula real con los datos de esta pregunta.":heightGuide?"Empieza por SIN: vas a introducir la fórmula real con l, d y P.":mode===3?"Calculadora libre. Haz la conversión y después pulsa «Revisar resultado».":"La pantalla ya contiene 44,895188. Empieza por la operación indicada.";render()}
 ...
 function guidedHeightPress(key,value){
  ...
  if(cursor===61)feedbackEl.textContent=height296?"La fórmula completa ha dado sen(ae) = 0,779722. Ahora recupera la altura estimada con SHIFT y SIN.":"La fórmula completa ha dado sen(ae) = 0,7058120802. Ahora recupera el ángulo con SHIFT y SIN.";
  if(cursor===66)feedbackEl.textContent=height296?"Ya tienes ae = 51,235123°. Pulsa °′″ para verla en grados, minutos y segundos.":"Ya tienes ae = 44,895188°. Ahora conviértelo al formato de las respuestas.";

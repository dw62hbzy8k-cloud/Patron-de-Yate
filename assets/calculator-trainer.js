...
function mount(root){
 ...
 function guidedHeightPress(key,value){
  ...
  if(cursor===formulaEnd)feedbackEl.textContent=pole731?"Resultado: −25°09′18″. Añade 360° para escribir hL entre 0° y 360°.":`La fórmula completa ha dado sen(ae${height58||height1335||height1258?'*':''}) = ${milestones[0].value}. Ahora recupera la altura estimada con SHIFT y SIN.`;
  if(cursor===inverseEnd)feedbackEl.textContent=pole731?"Ya tienes hL = 334°50′42″. Ahora resta este valor a 360° para obtener el arco corto P.":`Ya tienes ae${height58||height1335||height1258?'*':''} = ${milestones[1].value.replace('.',',')}°. Pulsa °′″ para verla en grados, minutos y segundos.`;

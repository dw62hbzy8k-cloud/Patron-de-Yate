...
function recommendedMode(progress){return progress.guided<2?1:2}
...
function buildZenithGuide(guideId){
 if(!/^zenith-\d+$/.test(guideId||""))return null;const id=guideId.slice(7),s=window.ZENITH_DISTANCE_SOLUTIONS&&window.ZENITH_DISTANCE_SOLUTIONS[id];if(!s)return null;
 const directions=[],milestones=[];let expected=[];
 if(s.inherited){
  expected=poleAngleKeys([29,"31.7"]);directions.push(...expected.map(key=>`Pulsa ${key}.`));directions[0]="Empieza escribiendo la altura instrumental ai = 29°31,7′.";
  s.correctionOps.forEach(function(correction){const keys=poleAngleKeys(correction.keys);expected=expected.concat([correction.key],keys,["="]);directions.push(`Aplica ${correction.label}: pulsa ${correction.key}.`,...keys.map(key=>`Pulsa ${key}.`),"Pulsa = para aplicar la corrección.");milestones.push({at:expected.length,value:correction.screen})});
 }else{
  expected=[];
 }
 const formulaEnd=expected.length,start=expected.length,ninety=["9","0","°′″"],avKeys=poleAngleKeys(s.avKeys);
 expected=expected.concat(ninety,["−"],avKeys,["=","°′″"]);
 directions.push("Escribe 90°: pulsa 9.","Completa 90°: pulsa 0.","Pulsa °′″ para indicar que son grados.",`Pulsa − para restar av = ${s.av}.`,...avKeys.map(key=>`Pulsa ${key}.`),"Pulsa = para obtener el valor de la distancia cenital.","Pulsa °′″ para verla en grados, minutos y segundos.");
 directions[start]=s.inherited?"Ya tienes av = 29°44,2′. Escribe 90° para calcular z = 90° − av.":"Escribe 90° para comenzar z = 90° − av.";
 const inverseEnd=expected.length-1;milestones.push({at:inverseEnd,value:s.zScreen},{at:expected.length,value:s.zScreen});
 return {kind:"zenith-distance",id,s,expected,directions,formulaEnd:formulaEnd||inverseEnd,inverseEnd,milestones,storageKey:`aprobarNautica_zenithDistanceTrainer_${id}_v1`,finalText:`Valor: |z| = ${s.z}. Como el Sol queda cara al Sur, el resultado completo es z = ${s.signedZ}. Respuesta ${s.answer}.`,subtitle:`Cálculo interactivo de ${s.title} · distancia cenital.`,objective:`Objetivo: z = ${s.signedZ} cara al Sur · respuesta ${s.answer}`,initial:s.inherited?"Empieza con ai = 29°31,7′; la guía aplicará las correcciones y después calculará z.":`Empieza escribiendo 90° y resta av = ${s.av}.`,firstFeedback:s.inherited?`Altura verdadera terminada: av = ${s.av}. Ahora calcula z = 90° − av.`:`Valor de la distancia cenital terminado: |z| = ${s.z}.`,secondFeedback:`La pantalla muestra ${s.zScreen}. El último paso es asignar el signo: cara al Sur significa z negativa.`,steps:s.inherited?"<li>Corregir ai y obtener av.</li><li>Calcular |z| = 90° − av.</li><li>Asignar signo negativo por quedar cara al Sur.</li>":"<li>Calcular |z| = 90° − av.</li><li>Leer grados, minutos y segundos.</li><li>Asignar signo negativo por quedar cara al Sur.</li>"};
}
...
function buildOrthodromicGuide(guideId){
 ...
 if(s.kind==="distance"){
  ...
  if(s.kind==="course"){
   const signedAngle=(parts,negative)=>negative?["(-)"].concat(angle(parts)):angle(parts),signedTrig=(name,parts,negative)=>[name].concat(signedAngle(parts,negative),[")"]),c=s.calc,storedAngle=name=>[name,"Ans",")"];
   const distanceFormula=signedTrig("sin",c.a,c.aNeg).concat(["×"],signedTrig("sin",c.b,c.bNeg),["+"],signedTrig("cos",c.a,c.aNeg),["×"],signedTrig("cos",c.b,c.bNeg),["×"],trig("cos",c.dl),["="]);
   const formulaEnd=distanceFormula.length,distanceAngle=distanceFormula.concat(["SHIFT","cos","Ans",")","="]),angleEnd=distanceAngle.length;
   const courseFormula=["AC","("].concat(signedTrig("sin",c.b,c.bNeg),["−"],signedTrig("sin",c.a,c.aNeg),["×"],storedAngle("cos"),[")","÷","("],signedTrig("cos",c.a,c.aNeg),["×"],storedAngle("sin"),[")","="]);
   const courseStart=distanceAngle.length,courseFormulaEnd=courseStart+courseFormula.length,courseAngleEnd=courseFormulaEnd+5,quadrant=s.deltaDirection==="W"?["3","6","0","−","Ans","="]:[];
   const expected=distanceAngle.concat(courseFormula,["SHIFT","cos","Ans",")","="],quadrant),directions=expected.map(key=>`Pulsa ${key}.`);
   directions[0]=`Pulsa SIN para comenzar la fórmula con lA = ${s.aMath}.`;
   directions[formulaEnd]="Ya tienes cos(D°). Pulsa SHIFT y después COS para recuperar el ángulo central D°.";
   directions[courseStart]="D° terminado. Pulsa AC para limpiar la pantalla y comenzar la fórmula del rumbo inicial.";
   directions[courseStart+1]="Abre paréntesis para escribir completo el numerador de la fórmula del rumbo.";
   expected.forEach((key,index)=>{if(index>courseStart&&index<courseFormulaEnd&&key==="Ans")directions[index]="Pulsa Ans para reutilizar D° directamente, sin copiar su número decimal."});
   directions[courseFormulaEnd]="Ya tienes cos(Ri). Pulsa SHIFT y después COS para obtener la abertura respecto al Norte.";
   if(s.deltaDirection==="W")directions[courseAngleEnd]="La llegada queda al Oeste: escribe 360 y resta Ans para colocar el rumbo en el cuadrante correcto.";
   const milestones=[{at:formulaEnd,value:c.cos},{at:angleEnd,value:c.angle},{at:courseStart+1,value:"0"},{at:courseFormulaEnd,value:c.courseCos},{at:courseAngleEnd,value:c.opening}];
   if(quadrant.length)milestones.push({at:expected.length,value:c.course});
   return {kind:"orthodromic-course",id,s,expected,directions,formulaEnd,inverseEnd:courseAngleEnd,milestones,storageKey:`aprobarNautica_orthodromicCourseTrainer_${id}_v2`,finalText:`Ri = ${s.officialCourse}. Respuesta ${s.answer}.`,subtitle:`Cálculo interactivo de ${s.title} · rumbo inicial ortodrómico.`,objective:`Objetivo: Ri = ${s.officialCourse} · respuesta ${s.answer}`,initial:`Empieza por la fórmula auxiliar de D°: pulsa SIN e introduce lA = ${s.aMath}.`,firstFeedback:`La parte derecha ha dado cos(D°) = ${s.distanceCos}. Ahora recupera D° con SHIFT y COS.`,secondFeedback:s.deltaDirection==="W"?`Abertura obtenida: ${s.courseOpening}°. Como la llegada queda al Oeste, colócala en su cuadrante restándola a 360°.`:`Abertura obtenida: ${s.courseOpening}°. Como la llegada queda al Este, este mismo valor es Ri.`,steps:"<li>Calcular el ángulo central auxiliar D°.</li><li>Calcular cos(Ri) reutilizando D° con Ans.</li><li>Colocar Ri en el cuadrante indicado por ΔL.</li>"};
  }
 const a=[43,"09.6"],b=[6,"48.0"],dl=[48,"20.6"],angleFromDistance=name=>[name,"Ans","÷","6","0",")"];
 ...
 const courseFormula=["AC","("].concat(trig("sin",b),["−"],trig("sin",a),["×"],angleFromDistance("cos"),[")","÷","("],trig("cos",a),["×"],angleFromDistance("sin"),[")","="]);
 const courseStart=distanceMiles.length,courseFormulaEnd=courseStart+courseFormula.length,courseAngleEnd=courseFormulaEnd+5;
 const expected=distanceMiles.concat(courseFormula,["SHIFT","cos","Ans",")","="],["3","6","0","−","Ans","="]),directions=expected.map(key=>`Pulsa ${key}.`);
 directions[0]="Pulsa SIN para empezar la fórmula de la distancia con lA = 43°09,6′.";
 directions[formulaEnd]="Ya tienes cos(D°). Pulsa SHIFT y después COS para recuperar el ángulo central D°.";
 directions[angleEnd]="Ya tienes D° en grados. Pulsa × 60 para convertir el arco en millas náuticas.";
 directions[courseStart]="Distancia terminada. Pulsa AC para limpiar la pantalla y comenzar el rumbo inicial.";
 directions[courseStart+1]="Abre paréntesis para escribir completo el numerador de la fórmula del rumbo.";
 const savedAngleUses=[];expected.forEach((key,index)=>{if(index>courseStart&&index<courseFormulaEnd&&key==="Ans")savedAngleUses.push(index)});
 savedAngleUses.forEach(index=>directions[index]="Pulsa Ans: contiene la distancia 3.346,600492. A continuación la dividirás entre 60 para recuperar D° sin copiar 55,7766749.");
 directions[courseFormulaEnd]="Ya tienes cos(Ri). Pulsa SHIFT y después COS para obtener la abertura respecto al Norte.";
 directions[courseAngleEnd]="Como el destino está al Oeste, escribe 360 y resta Ans para colocar el rumbo en su cuadrante.";
 return {kind:"orthodromic",id,s,expected,directions,formulaEnd,inverseEnd,milestones:[{at:formulaEnd,value:"0.5624200360"},{at:angleEnd,value:"55.7766749"},{at:inverseEnd,value:"3346.600492"},{at:courseStart+1,value:"0"},{at:courseFormulaEnd,value:"−0.4415409022"},{at:courseAngleEnd,value:"116.2022382"},{at:expected.length,value:"243.7977618"}],storageKey:`aprobarNautica_orthodromicTrainer_${id}_v2`,finalText:"Dort = 3.346,6 millas y Ri = 244°. Respuesta C.",subtitle:`Cálculo interactivo de ${s.title} · distancia y rumbo inicial ortodrómicos.`,objective:"Objetivo: Dort = 3.346,6 millas · Ri = 244°",initial:"Empieza por la fórmula de distancia: pulsa SIN e introduce lA = 43°09,6′.",firstFeedback:"La parte derecha ha dado cos(D°) = 0,5624200360. Ahora recupera D° con SHIFT y COS.",secondFeedback:"Distancia terminada: 3.346,6 millas. Queda guardada en Ans. Pulsa AC: Ans no se borra y la reutilizaremos dividida entre 60 para recuperar D°.",steps:"<li>Calcular el ángulo central D° con la fórmula de distancia.</li><li>Convertir D° en millas: D° × 60.</li><li>Calcular Ri reutilizando D° sin copiar sus decimales.</li>"};
}
function buildReclassifiedGuide(guideId){
 const source=window.RECLASSIFIED_CALCULATOR_GUIDES&&window.RECLASSIFIED_CALCULATOR_GUIDES[guideId];
...
function buildTimeGuide(guideId){
 const source=window.TIME_CALCULATOR_GUIDES&&window.TIME_CALCULATOR_GUIDES[guideId];
 if(!source)return null;
 return Object.assign({},source,{expected:[...source.expected],directions:[...source.directions],milestones:(source.milestones||[]).map(item=>Object.assign({},item))});
}
...
function setupDocking(root,calculationZone){
 ...
 const questionBoundary=root.closest("#qbox")||calculationZone;
 ...
 function setDocked(next){
  ...
  if(next){
   const dockHeight=Math.max(1,dock.getBoundingClientRect().height),trainerRect=root.getBoundingClientRect(),pageY=window.scrollY;
   ...
   exitScroll=Math.max(enterScroll+120,pageY+boundaryRect.bottom-80);
  }else{
 ...
 function updateDock(){
  ...
  requestAnimationFrame(function(){
   ...
   if(docked){
    ...
    if(pageY>=exitScroll){setDocked(false);blockRedockAtBottom=true;return}
    return
   }
...
function mount(root){
 ...
 if(inlineTrainer){root.classList.add("calc-inline-trainer");const compactControls=root.querySelector(".calc-trainer-controls"),dragHandle=root.querySelector("[data-ct-drag-handle]");if(compactControls&&dragHandle)dragHandle.insertAdjacentElement("afterend",compactControls);const calculationZone=root.closest("[data-calculation-zone]")||root.closest('div[style*="border:3px solid #1c6ea4"]')||root.closest('div[style*="border:3px solid #d6a31c"]')||root;setupDocking(root,calculationZone)}
 ...
 // Dentro de una pregunta se abre siempre el modo Aprender. El progreso y la
 // recomendación se conservan, pero el cambio de nivel lo decide el alumno.
 let progress=loadProgress(storageKey),mode=inlineTrainer?1:recommendedMode(progress),cursor=0,history=[],expression=startExpression,display=startValue,stored=Number(startValue),pending=null,typing=false,completed=false;
 ...
 function updateProgress(){const recommended=recommendedMode(progress);progressEl.innerHTML=`<span class="calc-progress-pill">Aprender: ${progress.guided}</span><span class="calc-progress-pill">Práctica: ${progress.practice}</span><span class="calc-progress-pill"><b>Recomendado: nivel ${recommended}</b></span>`}
...
function activeExpected(){return expected}
 ...
 function render(){const route=activeExpected();expressionEl.textContent=expression;resultEl.textContent=pretty(display);expressionEl.scrollLeft=expressionEl.scrollWidth;resultEl.scrollLeft=resultEl.scrollWidth;instructionEl.textContent=cursor<route.length?directions[cursor]:heightGuide?`Cálculo completo: ${finalHeightText}`:"Ejercicio terminado: 44,895188° = 44°53,7′.";updateSteps();updateKeyState();updateProgress()}
 function complete(){if(completed)return;completed=true;if(mode===1)progress.guided++;else progress.practice++;saveProgress(progress,storageKey);updateProgress()}
...
function updateHeightState(){
 ...
 function firstMismatch(route=expected){const limit=Math.max(history.length,route.length);let mismatch=0;while(mismatch<limit&&history[mismatch]===route[mismatch])mismatch++;return mismatch}
 function guidedHeightPress(key,value){
  if(value==="AC"&&expected[cursor]!=="AC"){setMode(mode);return}
...
function soloHeightPress(key,value){
 ...
 function review(){if(heightGuide){const route=activeExpected(),mismatch=firstMismatch(route);if(mismatch===route.length&&history.length===route.length){display=milestones[milestones.length-1].value;complete();render();return}expression=`ERROR · tecla ${mismatch+1}`;display=`${history[mismatch]||"ninguna"} ≠ ${route[mismatch]||"terminar"}`;render();return}const numeric=Number(display);if(Math.abs(numeric-53.71128)<.00001){feedbackEl.className="calc-feedback ok";feedbackEl.textContent="Resultado correcto: 53,71128′, que se redondea a 53,7′. Tu camino también es válido.";complete();return}const limit=Math.max(history.length,expected.length);let mismatch=0;while(mismatch<limit&&history[mismatch]===expected[mismatch])mismatch++;const pressed=history[mismatch]||"ninguna tecla",wanted=expected[mismatch]||"terminar";feedbackEl.className="calc-feedback bad";feedbackEl.textContent=`El primer desvío está en la pulsación ${mismatch+1}: pulsaste ${pressed}; en esta ruta correspondía ${wanted}. Reinicia y prueba otra vez.`}
 function setMode(next){mode=Number(next)===2?2:1;root.dataset.mode=String(mode);modeButtons.forEach(function(button){button.setAttribute("aria-pressed",String(Number(button.dataset.ctMode)===mode))});modeDescriptionEl.innerHTML=modeText[mode];levelEl.textContent=mode===1?"APRENDER 1":"PRÁCTICA 2";reset()}
 keys.forEach(function(key){key.addEventListener("click",function(){const value=key.dataset.key;if(heightGuide)guidedHeightPress(key,value);else guidedPress(key,value)})});modeButtons.forEach(function(button){button.addEventListener("click",function(){setMode(button.dataset.ctMode)})});root.querySelector("[data-ct-reset]").addEventListener("click",function(){setMode(mode)});setMode(mode)
}

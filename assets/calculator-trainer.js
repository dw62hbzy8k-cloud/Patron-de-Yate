...
function buildOrthodromicGuide(guideId){
 ...
 if(s.kind==="distance"){
  ...
  if(s.kind==="course"){
   const signedAngle=(parts,negative)=>negative?["(-)"].concat(angle(parts)):angle(parts),signedTrig=(name,parts,negative)=>[name].concat(signedAngle(parts,negative),[")"]),c=s.calc,d=decimal(c.angle);
   const distanceFormula=signedTrig("sin",c.a,c.aNeg).concat(["×"],signedTrig("sin",c.b,c.bNeg),["+"],signedTrig("cos",c.a,c.aNeg),["×"],signedTrig("cos",c.b,c.bNeg),["×"],trig("cos",c.dl),["="]);
   const formulaEnd=distanceFormula.length,distanceAngle=distanceFormula.concat(["SHIFT","cos","Ans",")","="]),angleEnd=distanceAngle.length;
   const courseFormula=["AC","("].concat(signedTrig("sin",c.b,c.bNeg),["−"],signedTrig("sin",c.a,c.aNeg),["×"],trig("cos",d),[")","÷","("],signedTrig("cos",c.a,c.aNeg),["×"],trig("sin",d),[")","="]);
 ...
 const courseFormula=["AC","("].concat(trig("sin",b),["−"],trig("sin",a),["×"],trig("cos",d),[")","÷","("],trig("cos",a),["×"],trig("sin",d),[")","="]);
...
function trainerMarkup(guideId){
	 const poleGuide=buildPoleGuide(guideId),meridianGuide=buildMeridianGuide(guideId),trueAltitudeGuide=buildTrueAltitudeGuide(guideId),orthodromicGuide=buildOrthodromicGuide(guideId),reclassifiedGuide=buildReclassifiedGuide(guideId),dynamicGuide=poleGuide||meridianGuide||trueAltitudeGuide||orthodromicGuide||reclassifiedGuide;

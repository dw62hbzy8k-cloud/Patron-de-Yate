...
function mount(root){
 ...
 function guidedHeightPress(key,value){
  ...
  if(cursor===formulaEnd)feedbackEl.textContent=`La fórmula completa ha dado sen(ae${height58||height1335||height1258?'*':''}) = ${milestones[0].value}. Ahora recupera la altura estimada con SHIFT y SIN.`;
  if(cursor===inverseEnd)feedbackEl.textContent=`Ya tienes ae${height58||height1335||height1258?'*':''} = ${milestones[1].value.replace('.',',')}°. Pulsa °′″ para verla en grados, minutos y segundos.`;

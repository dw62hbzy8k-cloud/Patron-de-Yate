...
function mount(root){
 ...
 const freeInline=root.dataset.inlineFree==="1";
 ...
 const startValue=freeInline?"0":"44.895188",startExpression=freeInline?"":"44.895188";
 let progress=loadProgress(),mode=freeInline?3:recommendedMode(progress),cursor=0,history=[],expression=startExpression,display=startValue,stored=Number(startValue),pending=null,typing=false,completed=false;
 ...
 function reset(){cursor=0;history=[];expression=startExpression;display=startValue;stored=Number(startValue);pending=null;typing=false;completed=false;feedbackEl.className="calc-feedback";feedbackEl.textContent=freeInline?"":mode===3?"Calculadora libre. Haz la conversión y después pulsa «Revisar resultado».":"La pantalla ya contiene 44,895188. Empieza por la operación indicada.";render()}

...
function buildMeridianGuide(guideId){
 ...
 return {kind:"meridian",id,s,expected,directions,formulaEnd,inverseEnd:expected.length,milestones:[{at:formulaEnd,value:s.zScreen},{at:expected.length,value:s.latScreen}],storageKey:`aprobarNautica_meridianLatitudeTrainer_${id}_v1`,finalText:`l = ${s.lat}.`,subtitle:`Cálculo interactivo de ${s.title} · latitud por meridiana.`,objective:`Objetivo: l = ${s.lat}`,initial:`Empieza calculando z: escribe 90° y resta av = ${s.av}.`,firstFeedback:`Distancia cenital terminada: z = ${s.z}. Ahora aplica la declinación ${s.decDir==="N"?"Norte sumando":"Sur restando"} ${s.decMagnitude}.`,steps:"<li>Calcular z = 90° − av.</li><li>Aplicar la declinación con su signo.</li><li>Leer la latitud final en grados y minutos.</li>"};
}
function trainerMarkup(guideId){
 const poleGuide=buildPoleGuide(guideId),meridianGuide=buildMeridianGuide(guideId),dynamicGuide=poleGuide||meridianGuide;

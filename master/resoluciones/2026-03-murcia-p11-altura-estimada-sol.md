...
3. `hL` se expresa sobre el círculo completo, de 0° a 360°. `P` utiliza el arco menor, de 0° a 180°, e indica Este u Oeste. Si `hL` está entre 0° y 180°, `P = hL W`; si `hL > 180°`, `P = 360° − hL E`. En esta pregunta: `P = 360° − 334°32,5′ = 25°27,5′ E`.
   ...
   Debajo de los dos casos se conserva un recordatorio de los signos de comparación para principiantes: `≤` significa «menor o igual que» y `>` significa «mayor que». Regla visual: la boca abierta mira al número mayor y la punta señala al menor. Aplicado aquí, en `hL ≤ 180°` la boca mira a `180°`; en `hL > 180°` la boca mira a `hL`.

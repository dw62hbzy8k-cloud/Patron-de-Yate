...
3. `hL` se expresa sobre el círculo completo, de 0° a 360°. `P` utiliza el arco menor, de 0° a 180°, e indica Este u Oeste. Si `hL` está entre 0° y 180°, `P = hL W`; si `hL > 180°`, `P = 360° − hL E`. En esta pregunta: `P = 360° − 334°32,5′ = 25°27,5′ E`.
   ...
   El gráfico debe mostrar simultáneamente el arco largo azul `hL = 334°32,5′ W`, el arco corto naranja `P = 25°27,5′ E`, el meridiano superior en `0°/360°`, la media circunferencia en `180°` y el semicírculo horario del Sol. Las etiquetas `hL` y `P` se sitúan fuera del círculo para no taparlo y cada una lleva una flecha del mismo color que toca su arco correspondiente. La combinación azul/naranja se utiliza para que los dos recorridos se distingan con claridad. La explicación acompaña el gráfico en cuatro pasos y compara los dos casos generales: `hL ≤ 180°` y `hL > 180°`.

   Debajo de los dos casos se conserva un recordatorio de los signos de comparación para principiantes: `≤` significa «menor o igual que» y `>` significa «mayor que». Regla visual: la boca abierta mira al número mayor y la punta señala al menor. La raya inferior no determina si es mayor o menor: añade «o igual». Por eso `≤` es «menor o igual», `≥` es «mayor o igual» y los símbolos `<` y `>` sin raya no admiten igualdad. Aplicado aquí, en `hL ≤ 180°` la boca mira a `180°` y se admite exactamente `180°`; en `hL > 180°` la boca mira a `hL` y este debe superar realmente `180°`.

   Inmediatamente debajo del gráfico se definen otra vez los símbolos: `hL` es el horario local del Sol, recorrido completo contado al Oeste y resultado intermedio; `P` es el ángulo en el Polo, arco más corto de 0° a 180° y valor que se introduce en la fórmula. El punto central se llama simplemente «Polo celeste» en el gráfico principal. En la explicación secundaria se aclara que puede aparecer como `PN` (Polo Norte celeste) o `PS` (Polo Sur celeste) según el hemisferio, pero que esas abreviaturas no se necesitan para resolver esta pregunta. Se advierte que `PN`/`PS` nombran un punto y `P` es un ángulo.
